...
GCGraph=select(GC,ID,Species,GCP,ATP)  
#the instructions say to create a scattersheet for gc% even though you need two numerical columns for a scatter sheet? not quite sure what we're supposed to use for the second numerical value, and it's too late to ask now, so I threw in AT%. This of course just creates a line, but it's a working graph at least ¯\_(ツ)_/¯

qplot(ATP,GCP,colour=Species,data=GC,ylab = "AT%", xlab = "GC%", alpha=I(0.7)) + theme_classic()
```
